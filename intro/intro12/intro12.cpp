...
int main()
{//local(지역){} 지역변수는 이 대괄호 안에서만 적용되고 대괄호가 닫히면 지역변수를 없어진다
    //
    short num;  //메모리 2 바이트 확보(정수형으로, 시스템이 알아서 잡으세요 라는 뜻)
    num = 5;    //확보된 메모리num공간에 정수 5를 대입하라
    ...
    short* addr = &num; //short *(포인터) 형을 갖는 변수 addr을 선언한고 num의 주소를 저장한다
    printf("addr의 크기: %u \n", sizeof(addr));
    printf("addr의 주소: %p\n", &addr);
